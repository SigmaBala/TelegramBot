...
from MyTgBot.database.afk_db import add_afk, is_afk, remove_afk
from MyTgBot import bot
from MyTgBot.help.readable_time import get_readable_time2
...
async def active_afk(self: Client, ctx: Message):
    ...
    if len(ctx.command) == 1 and not ctx.reply_to_message:
        ...
        await bot.download_media(ctx.reply_to_message, file_name=f"{user_id}.jpg")
        details = {
            "type": "photo",
            "time": time.time(),
            "data": None,
